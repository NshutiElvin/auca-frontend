              ...
              <>
                ...
                <Card className="border-2">
                  <CardHeader className="bg-gradient-to-r from-orange-500/5 to-transparent">
                    ...
                    <CardDescription>
                      Controls whether courses are sorted by student count before scheduling begins.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="pt-6 space-y-4">
                    <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-accent/50 transition-colors">
                      <div className="flex-1">
                        <Label htmlFor="prioritize-large" className="cursor-pointer font-medium flex items-center gap-2">
                          Prioritize Large Courses
                          <Badge variant="destructive" className="text-xs">Enforced</Badge>
                        </Label>
                        <p className="text-sm text-muted-foreground mt-1">
                          Schedule courses with the most students first. Gives large groups first pick
                          of dates and slots, reducing the chance they end up unscheduled.
                        </p>
                      </div>
                      <Switch
                        id="prioritize-large"
                    ...
                    <Alert>
                      <AlertCircle className="h-4 w-4" />
                      <AlertDescription className="text-sm">
                        Other course settings (allow_split_groups, allow_course_group_combinations,
                        detect_student_conflicts_automatically, build_compatibility_graph,
                        use_greedy_coloring_algorithm) are stored but not read by the scheduling function.
                      </AlertDescription>
                    </Alert>
                  </CardContent>
                </Card>
                ...
                <Card className="border-2">
                  <CardHeader className="bg-gradient-to-r from-purple-500/5 to-transparent">
                    ...
                    <CardDescription>
                      Configure preferred slot order and priority for each student group (A–F).
                      When a course-group contains students from multiple groups, the group with the
                      lowest priority number wins and its slot order is applied for the entire course-group.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="pt-6 space-y-4">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <Input
                                    ...
                                    onChange={(e: { target: { value: string; }; }) => updateGroupPriority(group, parseInt(e.target.value))}
                                    min="1"
                                    className="w-16 h-7 text-xs"
                                  />
                                  <span className="text-xs text-muted-foreground">(lower wins)</span>
                                </div>
                              </div>

                              {/* Slot order with arrows */}
                              <div className="space-y-2">
                                <Label className="text-sm font-medium">Slot Preference Order</Label>
                                <div className="flex flex-col gap-1">
                                  {prefs.slots_order.map((slot: string, idx: number) => (
                                    <div key={slot} className="flex items-center gap-2">
                                      <Badge
                                        variant={idx === 0 ? "default" : "outline"}
                                        className="w-6 h-6 rounded-full flex items-center justify-center p-0 text-xs shrink-0"
                                      >
                                        {idx + 1}
                                      </Badge>
                                      <span className="text-sm flex-1">{slot}</span>
                                      <div className="flex gap-1">
                                        <Button
                                          ...
                                          className="h-6 w-6"
                                          disabled={idx === 0}
                                          onClick={() => moveSlot(group, idx, idx - 1)}
                                        >
                                        ...
                                        <Button
                                          ...
                                          variant="ghost"
                                          className="h-6 w-6"
                                          disabled={idx === prefs.slots_order.length - 1}
                                          onClick={() => moveSlot(group, idx, idx + 1)}
                                        >
                    ...
                    <Alert className="border-purple-200 bg-purple-50 dark:bg-purple-950/20">
                      ...
                      <AlertDescription className="text-sm text-purple-900 dark:text-purple-100">
                        Slot names must exactly match those defined in Time Slots.
                        Priority determines which group's preference applies when multiple groups
                        share a course-group — lower number wins.
                      </AlertDescription>
                    </Alert>
                  </CardContent>
                </Card>
