      ...
      <AnimatePresence initial={false} custom={direction} mode="wait">
        ...
        >
          ...
          <div className="col-span-7 flex flex-col relative">
            ...
            >
              {daysOfWeek.map((day: Date, idx: number) => (
                    ...
                    <div className="text-center p-4">
                      ...
                      <div 
                        ...
                        onClick={(e: React.MouseEvent<HTMLDivElement>) => {
                          ...
                          const dayEvents: Event[] | undefined = getters.getEventsForDay(
                            day.getDate(),
                            currentDate
                          );
                                      ...
                                      <div className="grid grid-cols-[100px_1fr] h-full">
                                        ...
                                        <div className="relative">
                                          ...
                                          {dayEvents.map((event: Event) => {
                                            ...
                                            const timeGroups: Event[][] = groupEventsByTimePeriod(dayEvents);
                                            
                                            // Find which time group this event belongs to
                                            let eventsInSamePeriod: number = 1;
                                            let periodIndex: number = 0;
                                            
                                            for (let i = 0; i < timeGroups.length; i++) {
                                              const groupIndex = timeGroups[i].findIndex((e: Event) => e.id === event.id);
                                              if (groupIndex !== -1) {
                                            ...
                                            const { height, top, left, maxWidth, minWidth }: EventStyling = handlers.handleEventStyling(
                                              event,
                                              dayEvents,
                                              {
                                                eventsInSamePeriod,
                  ...
                  >
                    <AnimatePresence initial={false}>
                      ...
                      {visibleEvents?.map((event: Event, eventIndex: number) => {
                        ...
                        // Find which time group this event belongs to
                        for (let i = 0; i < timeGroups.length; i++) {
                          const groupIndex: number = timeGroups[i].findIndex((e: Event) => e.id === event.id);
                          if (groupIndex !== -1) {
                            eventsInSamePeriod = timeGroups[i].length;
                            periodIndex = groupIndex;
                            break;
                        ...
                        const { height, left, maxWidth, minWidth, top, zIndex }: EventStyling =
