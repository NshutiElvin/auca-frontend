...
export function OccupanciesPage() {
  ...
  const [selectedOccupancies, setSelectedOccupancies] = React.useState<
    RoomOccupancy[]
  >([]);
      ...
      <Dialog open={showModal} onOpenChange={handleCloseModal}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {formatDate(
                selectedOccupancies[0]?.date
                  ? new Date(selectedOccupancies[0]?.date)
                  : new Date()
              )}
            </DialogTitle>
            <DialogDescription>Room Occupancy Overview</DialogDescription>
          </DialogHeader>
          ...
          <ScrollArea className="h-[400px] rounded-md border p-4">
            {groupedOccupancies.map((slot, index) => (
              <div
                key={index}
                className="space-y-3 py-3 border-b border-gray-600 pt-2"
              >
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {slot.total_students}/{slot.room_capacity} seats occupied
                      (
                      {(
                ...
                <div className="w-full bg-white-700 rounded h-12 overflow-hidden border">
                  <div className="flex h-full">
                    {slot.courses.map((course: any, courseIndex: number) => {
                      const capacityPercentage =
                        (course.student_count / slot.room_capacity) * 100;
                      const colors = [
                        "bg-gray-500",
                        "bg-orange-350",
                        "bg-purple-500",
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-black text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                            {course.course_code} {course.course_semester}{" "}
                            {course.course_group}: {course.student_count}{" "}
                            students ({capacityPercentage.toFixed(1)}%)
                          </div>
                        </div>
                      );
                    })}
                    ...
                    {slot.total_students < slot.room_capacity && (
                      <div
                        ...
                        style={{
                          width: `${
                            ((slot.room_capacity - slot.total_students) /
                              slot.room_capacity) *
                            100
                          }%`,
                        }}
                  ...
                  {slot.courses.map((course: any, courseIndex: number) => (
                    ...
                    >
                      ...
                      <span className="text-xs">
                        {course.course_code} {course.course_semester}{" "}
                        {course.course_group}: {course.student_count} students
                      </span>
                    </div>
                  ))}
                </div>
