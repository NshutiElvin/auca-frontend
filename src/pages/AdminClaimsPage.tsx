...
import {
  RefreshCw,
  Inbox, Clock, CheckCircle2, XCircle, LayoutList,
} from 'lucide-react';
...
export const AdminClaimsPage: React.FC = () => {
  ...
  const { data: filteredClaims = [], isLoading, refetch: refetchFiltered } = useClaims({
          ...
          {STATUS_PILLS.map(({ value, label, icon: Icon, dot }) => {
            ...
            return (
              ...
              >
                ...
                {dot && (
                  <span className={`block h-1.5 w-1.5 rounded-full flex-shrink-0 ${dot}`} />
                )}
                ...
                {/* Count badge */}
                {count > 0 && (
                  <span className={`tabular-nums text-[10px] font-semibold leading-none ${active ? 'opacity-60' : ''}`}>
                    {count}
                  </span>
                )}

                {/* "New" indicator dot on pending (when not active) */}
                {value === ClaimStatus.PENDING && newCount > 0 && !active && (
