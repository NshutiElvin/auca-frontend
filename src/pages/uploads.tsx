...
const BulkUpload = () => {
  ...
  // ── File reading ────────────────────────────────────────────────────────────
  // Only reads the FIRST data row for preview.
  // This is intentional — we just need to confirm columns + auto-select the
  // semester term. The backend reads the full file independently.
    ...
    reader.onload = (e) => {
      try {
        ...
        setTimeout(() => {
          ...
          // ── Preview: first row only ──────────────────────────────────────
          const firstRow = rows[0];
          const previewItem: PreviewDataItem = { id: 0, selected: true };
          headers.forEach((header, i) => {
            previewItem[header] = firstRow[i] !== undefined ? firstRow[i] : "";
          });
          setPreviewData([previewItem]);

          // ── Auto-select semester from the first row's TERM value ─────────
          // The backend discovers all unique terms from the full file itself,
          // so we only need the one value visible in the preview row to
          // pre-populate and auto-select the dropdown.
          const termValue = previewItem["TERM"];
          if (termValue !== undefined && termValue !== null) {
            const termStr = String(termValue).trim();
            if (termStr.length) {
              setTerms([termStr]);
          ...
          setFileProcessing({
            status: "success",
            message: `File ready — ${totalRows.toLocaleString()} rows detected`,
            progress: 100,
          });
          setShowPreview(true);
        }, 600);
  ...
  const processSseLine = useCallback((raw: string) => {
    if (!raw.startsWith("data:")) return;
    try {
      const event = JSON.parse(raw.replace(/^data:\s*/, "")) as SSEEvent;
      ...
      if (event.type === "error") {
        setFileProcessing({ status: "error", message: event.message, progress: 0 });
      ...
      console.warn("Skipped unparseable SSE line:", raw);
    }
  }, [setToastMessage]);

  // ── SSE Upload — native fetch + ReadableStream ───────────────────────────────
  //
  // WHY NOT AXIOS:
  // axios with responseType:"text" uses XMLHttpRequest internally.
  // XHR buffers the entire response body and only fires onDownloadProgress
  // after the connection closes — so all SSE events arrive at once at the end,
  // giving the user an infinite spinner then a sudden jump to 100%.
  //
  // fetch + response.body.getReader() is the correct streaming API.
  // Each chunk is delivered to the while loop the moment Daphne flushes it.
  //
  // BUFFER LOGIC:
  // We accumulate bytes across chunks in `buffer`. Splitting on "\n\n" gives
  // us complete SSE events (all parts except the last). The last part is kept
  // in the buffer because it may be the start of the next event that hasn't
  // fully arrived yet. This handles the case where a JSON payload is split
  ...
  const handleUpload = async () => {
    ...
    try {
      // Use the same token that useUserAxios injects.
      // Adjust the localStorage key to match your auth setup if needed.
      const token = localStorage.getItem("access_token");

      const response = await fetch("/api/uploads/", {
        method: "POST",
        // Do NOT set Content-Type manually — the browser sets the correct
        // multipart/form-data boundary automatically when left unset.
        headers: {
      ...
      while (true) {
        ...
        // multi-byte sequences — they'll be completed in the next chunk
        buffer += decoder.decode(value, { stream: true });

        // Split on SSE event boundary.
        // parts[0..n-2] are complete events; parts[n-1] is the partial tail.
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="mb-10 space-y-2 flex justify-between items-center flex-wrap">
        ...
        {/* Semester selector — auto-populated + auto-selected from first row's TERM */}
        <div className="flex flex-col items-center justify-center">
          <h1>Select Semester</h1>
          <Select
            value={selectedTerm}
            disabled={terms.length === 0 || isRunning}
        ...
        <Card className="mb-8">
          <CardContent className="p-6">
            <div className="space-y-4">
              ...
              {/* Action buttons — only shown when file is parsed but import hasn't started */}
      ...
      {/* Data Preview Table — header + first row only */}
        ...
        <Card>
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Showing first row — confirming columns and semester term.
              </CardDescription>
            </div>
