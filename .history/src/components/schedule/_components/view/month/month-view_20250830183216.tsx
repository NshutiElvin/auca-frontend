  ...
  return (
    <div>
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b">
        <div className="grid grid-cols-7 gap-1 sm:gap-2 ml-4 py-2">
          {/* Empty cells for alignment */}
          {Array.from({ length: adjustedStartDay }).map((_, idx) => (
            <div key={`sticky-header-empty-${idx}`} className="h-6">
          ...
          {/* Additional headers for subsequent weeks */}
          {filteredDaysInMonth.length > (7 - adjustedStartDay) && 
            Array.from({ length: Math.min(7, filteredDaysInMonth.length - (7 - adjustedStartDay)) }).map((_, idx) => {
              ...
              return shouldShow ? (
                <div
                  key={`sticky-header-week2-${idx}`}
                  className="text-left text-lg font-medium text-muted-foreground"
                >
                  {daysOfWeek[dayIndex]}
                </div>
              ) : (
                <div key={`sticky-header-empty-week2-${idx}`} className="h-6"></div>
              );
            })
          }
        </div>
