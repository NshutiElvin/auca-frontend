  ...
  const getConfigurations=async()=>{
    try {
     ...
     setDefaultConfigurations(
      ...
      ...prev,
      term: resp.data.data.semesters[0]?.id?.toString() || "1",
      location: resp.data.data.locations[0]?.id?.toString() || "1"
    }));
    } catch (error) {
         setToastMessage({
        ...
        <DialogContent className="sm:max-w-[425px] md:max-w-[500px] max-h-[90vh] overflow-y-auto ">
          ...
          <DialogFooter>
            <Button variant={"default"} onClick={() => {
              if (configuration.academicYear.trim().length<4|| configuration.location.trim().length<=0|| configuration.location.trim().length<=0){
                setToastMessage({
            ...
            }} disabled={configuration.academicYear.trim().length<4|| configuration.location.trim().length<=0|| configuration.location.trim().length<=0}>
              Continue
            </Button>{" "}
