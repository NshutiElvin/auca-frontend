...
export default function CreateNewTimeTableModal() {
  ...
  const[isGeneratingTimetable, startGeneratingTimetableTransition]= useTransition()
    ...
    >
      <div className="flex flex-col items-center justify-start">
        <div className="relative flex items-center justify-between w-full px-4 py-2 mb-4 gap-4">
          ...
          <Button onClick={generateTimetable} size="sm" className="flex items-center gap-1 min-w-[110px]" disabled={selectedCourses.length<=0 || examSlots==null || isGeneratingTimetable}>
            {isGeneratingTimetable ?<Loader className="h-4 w-4" />:<Printer className="h-4 w-4" />}
            Generate
          </Button>
        </div>
        <div
