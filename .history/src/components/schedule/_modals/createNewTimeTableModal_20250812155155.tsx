...
export default function CreateNewTimeTableModal() {
  ...
  const [isGeneratingTimetable, startGeneratingTimetableTransition] =
    useTransition();
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              >
                <option disabled={examSlots[selectedSlot.date].find((s:any)=>s.name=="Morning")!=undefined} value="Morning">Morning</option>
                <option  disabled={examSlots[selectedSlot.date].find((s:any)=>s.name=="Afternoon")!=undefined} value="Afternoon">Afternoon</option>
                <option disabled={examSlots[selectedSlot.date].find((s:any)=>s.name=="Evening")!=undefined} value="Evening">Evening</option>
              </select>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="start-time" className="text-right">
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Popover>
                ...
                <PopoverContent className="w-auto p-0" align="center">
                  <Calendar
                    mode="single"
                    selected={new Date(newDay.date)}
                    onSelect={(selected) => {
                      if (selected) {
                        selected.setDate(selected.getDate() + 1);
                        const newDate = selected.toISOString().split("T")[0];
                        if (newDate in examSlots) {
                          setToastMessage({
                            message: "Day already scheduled",
                            variant: "danger",
                          });
                        } else {
                          setNewDay({
            ...
            <div className="flex flex-col gap-2">
              {newDay.slots.map((myslot, index) => {
                return (
                  <div
                    key={myslot.id}
                    className="flex flex-col gap-1 border shadow-md border-foreground rounded-md p-2"
                  >
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      >
                        <option disabled={newDay.slots.find((s:any)=>s.name=="Morning")!=undefined} value="Morning">Morning</option>
                <option  disabled={newDay.slots.find((s:any)=>s.name=="Afternoon")!=undefined} value="Afternoon">Afternoon</option>
                <option disabled={newDay.slots.find((s:any)=>s.name=="Evening")!=undefined} value="Evening">Evening</option>
                      </select>
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      <Label htmlFor="start-time" className="text-right">
                      ...
                      <Input
                        ...
                        id="start-time"
                        className="col-span-3"
                        value={newDay.slots[index].start || ""}
                        onChange={(e) => {
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        ...
                        disabled={!newDay.slots[index]}
                      />
                    </div>
                    {newDay.slots[index].start && newDay.slots[index].end && (
                      <div className="text-center text-sm text-muted-foreground">
                        Duration:{" "}
                        {calculateDuration(
          ...
          <DialogFooter>
            <Button
              type="submit"
              disabled={newDay.date.length <= 0 || newDay.slots.length <= 0}
              onClick={() => {
