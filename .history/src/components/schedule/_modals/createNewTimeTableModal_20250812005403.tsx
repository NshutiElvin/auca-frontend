...
export default function CreateNewTimeTableModal() {
  ...
  return (
    <div className="flex flex-col items-center  justify-start">
      <div className="flex justify-center gap-1 items-center border w-fit rounded-sm p-2 shadow-md">
        <Button
          variant={windowView.formOnly ? "default" : "secondary"}
          onClick={() => {
            setWindowView({ formOnly: true, split: false, slotsOnly: false });
          }}
        >
          Form Only
        </Button  >
        <Button variant={windowView.split ? "default" : "secondary"}  onClick={() => {
            setWindowView({ formOnly: false, split: true, slotsOnly: false });
          }}>
          Split
        </Button>
        <Button variant={windowView.slotsOnly ? "default" : "secondary"}  onClick={() => {
            setWindowView({ formOnly: false, split: false, slotsOnly: true });
          }}>
          Slots Only
        </Button>
      </div>
      <div className={`grid ${windowView.split?"grid-cols-2":"grid-cols-1"}`}>
        <form
          className={`flex flex-col gap-4 p-4 pt-0 sm:max-w-[800px] max-h-[100vh] overflow-y-auto`}
          onSubmit={handleSubmit(getExamsSlots)}
        >
          <div className="text-center py-4 px-3 text-2xl font-bold">
      ...
      <div>
          ...
          {examSlots ? (
            <div className="grid grid-cols-3 gap-2 max-h-[70vh] overflow-y-scroll">
            ...
            <div className="text-center py-4 px-3">
              {" "}
              No Dates selected yet! select start and end date and then generate
              slots
            </div>
          )}
        </div>
