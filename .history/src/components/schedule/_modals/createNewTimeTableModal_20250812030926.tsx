...
const addSlotToDate = (date: string, newSlot: { name: string; start: string; end: string }) => {
  ...
  if (dayOfWeek === 5) {
    ...
    if (hours >= 17) { 
      setToastMessage({message:"Friday slots cannot extend beyond 5:00 PM", variant:"danger"});
      return false;
    }
  }
 
  if (newSlot.end <= newSlot.start) {
     setToastMessage({message:"End time must be after start time", variant:"danger"});
    return false;
  }
                  ...
                  {Object.keys(examSlots).map((date: string, index) => {
                    ...
                    return (
                      <Card key={index} className="min-w-0">
                        {" "}
                        <div className="text-center py-3 px-2 border-b sticky top-0 bg-background z-10">
                          <CardTitle>
                            ...
                            >
                              ...
                              <div className="flex gap-1">
                                <Button
                                  ...
                                  onClick={()=>{
                                    setSelectedSlot({name:slot.name, date:date, id:slot.id, start:slot.start, end:slot.end})
                                    setDialogOpen(true)
                                  }}
                                >
