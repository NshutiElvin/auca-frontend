...
function ServerLoader() {
  ...
  return (
    ...
    serverLoadingMessage.isServerLoading && (
      <div className="flex  z-50  flex-col gap-3 p-0 m-0 rounded-lg shadow-lg border min-h-[100vh] top-0 min-w-full">
        <div
          className={cn(
            "md:top-4 md:right-4 md:translate-x-0 absolute top-0 right-5 flex items-center",
        ...
        {currentExamData && <div>{JSON.stringify(currentExamData)}</div>}
      </div>
    )
  );
}
