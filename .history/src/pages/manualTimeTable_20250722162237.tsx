...
import { Search, Clock, Calendar, X, Grip } from "lucide-react";
...
function ManualTimeTable() {
  ...
  const [isGettingUnscheduledExams, startGettingUnscheduledExamsTransition] =
    useTransition();
  ...
  const handleDroppedCourse= async(e: React.DragEvent, slotInfo:any)=>{
    e.preventDefault();
    try {
      setServerLoadingMessage({message:`Processing drop course ...`,isServerLoading:true})
      const resp= await axios.post("/api/exams/exams/add-exam-to-slot/", {...slotInfo, course: draggedCourse})
      ...
      setServerLoadingMessage({ isServerLoading:true})
    }
  }
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen} >
      <div className={`grid grid-cols-1 lg:grid-cols-3 gap-2 ${serverLoadingMessage?.isServerLoading && "pointer-events-none"}`}>
