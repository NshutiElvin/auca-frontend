...
function ManualTimeTable() {
  ...
  const handleDrop = (slotInfo: any) => {
    if (suggesstedSlot){
      console.log("Suggested Slot: ", suggesstedSlot);
      setSelectedSlotInfo((prev:any)=>{
        return {
          ...prev,
  ...
  useEffect(()=>{
    if (bestSuggestion)
      setSuggesstedSlot({date:bestSuggestion?.date, slot: bestSuggestion?.slot});
  },[bestSuggestion])
  return (
    <Dialog
      open={dialogOpen}
        ...
        <DialogContent className="sm:max-w-[600px] md:max-w-[800px] lg:max-w-[900px] max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader className="text-center space-y-3 pb-4 flex-shrink-0">
            ...
            <DialogDescription className="text-sm  text-center">
              The following conflicts prevent this course from being scheduled on the selected date and slot.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="flex-shrink-0 mt-6 p-4 border-t bg-gray-800">
            <div className="flex items-center justify-between w-full">
              <div>
                ...
                If you click "Yes", the exam will be scheduled in the best suggested slot unless you choose a different one above.
                </p>
              </div>
