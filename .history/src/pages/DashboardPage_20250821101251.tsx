...
//          let response=null;
//         if(selectedLocation)
//            response = await axios.get(`/api/schedules/dashboard?location=${selectedLocation?.id}`);
//         else{
...
//    const handleCardClick = async (cardType:any) => {
...
//       // Optionally navigate to a dedicated page or show modal
//       // navigate(`/admin/exams?filter=${cardType}`);
...
//       'Today\'s Completed Exams': 'completed-exams',
//       'Today\'s Expected Exams': 'scheduled-exams', // or create specific endpoint
...
//       <Card
//         key={index}
//         className="shadow-sm border cursor-pointer transition-all hover:shadow-md hover:border-purple-300"
...
//             </div>
//           </div>
//           {selectedCard === cardType && isLoadingCard && (
//             <div className="mt-2 text-xs text-purple-500">Loading...</div>
//           )}
//         </CardContent>
//       </Card>
//     );
...
//     const CardDataModal = () => {
//     if (!selectedCard || isLoadingCard) return null;

//     return (
//       <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
//         <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-11/12 max-w-4xl max-h-[80vh] overflow-y-auto">
...
//                 <h4 className="font-semibold">{item.name || item.exam_name}</h4>
//                 <p>Date: {item.date}</p>
  ...
  const handleCardClick = async (cardType: string) => {
    ...
    try {
      ...
      const endpointMap: any = {
        ...
        'popular-exams': '/api/schedules/popular-exams/',
        'today-students': '/api/schedules/today-students/',
        'upcoming-exams': '/api/schedules/upcoming-exams/',
        'recent-completed-exams': '/api/schedules/today-completed-exams/',
        'recent-expected-exams': '/api/schedules/today-expected-exams/'
      };
        ...
        <CardContent className="p-4">
          <div className="space-y-2">
            <h4 className="font-semibold text-lg">
              {item.name || item.exam_name || item.title || `Item ${index + 1}`}
            </h4>
            {item.date && (
              <p className="text-sm text-muted-foreground">
                <span className="font-medium">Date:</span> {format(new Date(item.date), 'MMMM d, yyyy')}
              </p>
            )}
            {item.start_time && item.end_time && (
              <p className="text-sm text-muted-foreground">
                <span className="font-medium">Time:</span> {item.start_time} - {item.end_time}
              </p>
            )}
            {item.student_count !== undefined && (
              <p className="text-sm text-muted-foreground">
                <span className="font-medium">Students:</span> {item.student_count}
              </p>
            )}
            {item.status && (
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
            <DialogHeader>
              ...
              <DialogDescription>
                {isLoadingCard ? 'Loading data...' : `Showing ${cardData.length} items`}
              </DialogDescription>
            </DialogHeader>
