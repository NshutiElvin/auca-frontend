...
function ManualTimeTable() {
  ...
  const [draggedCourseGroup, setDraggedCourseGroup] = useState<any>(null);
   const [backupdraggedCourseGroup, setBackupDraggedCourseGroup] = useState<any>(null);
  ...
  const [selectedSlotInfo, setSelectedSlotInfo] = useState<any | null>(null);
   const [backupSelectedSlotInfo, setBackupSelectedSlotInfo] = useState<any | null>(null);
  ...
  const[isReviewingConflict, startReviewingConflictTransition]= useTransition()
...
const reviewConflicts = () => {
  ...
  const scheduleCourse = async () => {
  if (!draggedCourse && !draggedCourseGroup && !existingDraggedExamGroup) return;
  ...
  try {
    ...
    if (resp.data.success) {
      ...
      if (existingDraggedExamGroup) {
        await getExams(); // Only needed for this case
      }
      handleDrop(selectedSlotInfo);
    } else {
      setToastMessage({ message: resp.data.message, variant: "danger" });
                    ...
                    <div className="flex-1 overflow-y-auto">
                      <div className="space-y-3 p-3">
                        {date.exams.map((slot, slotIdx) => (
                          ...
                          >
                            <div className="p-3 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 flex justify-between">
                              <h3 className="text-sm font-semibold   text-center ">
                                {slot.name}
                              </h3>
                              <div className="inline-flex items-center px-1 py-1.5 rounded-full text-sm font-medium bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 border border-blue-200 dark:border-blue-700">
                                {slot.start} - {slot.end}
                              </div>
                              <Button
                                variant={"ghost"}
  ...
  "
          ...
          {dialogOption?.option == "conflict" && (
            <DialogContent className="sm:max-w-[600px] md:max-w-[800px] lg:max-w-[900px] max-h-[90vh] flex-col">
              {isReviewingConflict &&    <div className="absolute top-[calc(90vh/2)] left-[calc(750px/2)] ">
              <Loader2 className="h-5 w-5 z-10 text-yellow-300 font-medium"/>
            </div>}
              <DialogHeader className="text-center space-y-3 pb-4 flex-shrink-0 p-2">
                <DialogTitle className="text-xl font-bold leading-tight text-center">
                  ...
                  {backupSelectedSlotInfo?.day
                    ? format(new Date(backupSelectedSlotInfo.day), "dd MMM yyyy")
                    : ""}
                </DialogTitle>
