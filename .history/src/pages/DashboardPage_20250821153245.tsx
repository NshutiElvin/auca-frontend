...
const renderCardDataItem = (item: any, index: number) => {
  return (
    <tr 
      key={index} 
      className="border-b border-gray-200 hover:bg-blue-50/60 hover:cursor-pointer transition-all duration-200 ease-in-out group"
    >
      ...
      <td className="py-4 px-3">
        <div className="font-semibold text-gray-800 text-base">
          {item.group.course.title || item.exam_name || item.title || `${index + 1}`}
        </div>
      </td>
      ...
      <td className="py-4 px-3">
        <div className="text-sm text-gray-600 font-medium">
          {item.start_time && item.end_time ? (
            <span className="bg-blue-100/70 text-blue-700 py-1 px-2.5 rounded-full text-xs font-medium">
              {item.start_time} - {item.end_time}
            </span>
          ) : '-'}
        </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                {isLoadingCard ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 space-y-3">
                    ...
                    <p className="text-xs text-muted-foreground/70">Try refreshing or check back later</p>
                  </div>
                )}
              </div>
            </div>
