  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-700">
        ...
        <p className="text-sm text-white">Total Submissions: <span className="text-purple-300">{totalSubmissions}</span></p>
          ...
          <ul className="text-xs space-y-1">
            {exams.map((exam: any, index: number) => (
              <li key={index} className="flex justify-between">
                <span className="text-gray-300 truncate max-w-[180px]">{exam.name}</span>
                <span className="text-purple-300 ml-2">{exam.student_count}</span>
              </li>
            ))}
          </ul>
        </div>
...
const barChartData =
  ...
  day: dayData.day,
  submissionsTotal: dayData.submissions.reduce((sum:any, exam:any) => sum + exam.student_count, 0),
