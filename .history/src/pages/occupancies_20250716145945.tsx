...
function handleShowRoomOccupancies(roomOccupancy: RoomOccupancy[]) {
  // Group by room and time slot combination
  // const groupedOccupancies = roomOccupancy.reduce((acc, occupancy) => {
  //   const slotKey = `${occupancy.room_name}_${occupancy.start_time}-${occupancy.end_time}`;
  ...
  // Generate grouped occupancies for modal display
  // const groupedOccupancies = React.useMemo(() => {
  //   if (selectedOccupancies.length === 0) return [];

  //   const grouped = selectedOccupancies.reduce((acc, occupancy) => {
  //     const roomKey = occupancy.room_name;
  ...
  //     acc[roomKey].total_students += occupancy.student_count;

  //     return acc;
  //   }, {} as Record<string, any>);

  //   return Object.values(grouped).map((room: any) => {
  //     const startTime = new Date(`2000-01-01 ${room.start_time}`);
  //     const endTime = new Date(`2000-01-01 ${room.end_time}`);
  //     const durationMs = endTime.getTime() - startTime.getTime();
  //     const durationHours = Math.floor(durationMs / (1000 * 60 * 60));
  //     const durationMinutes = Math.floor(
  //       (durationMs % (1000 * 60 * 60)) / (1000 * 60)
            ...
            return (
              ...
              >
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-black text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                  {course.course_code}: {course.student_count} students ({capacityPercentage.toFixed(1)}%)
                </div>
              </div>
            );
          })}
        ...
        {slot.courses.map((course: any, courseIndex: number) => (
          <div key={courseIndex} className="flex items-center space-x-2">
            ...
            <span className="text-xs">
              {course.course_code}: {course.student_count} students
            </span>
          </div>
        ))}
      </div>
