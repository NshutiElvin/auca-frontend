...
function ManualTimeTable() {
  ...
  const getPossibleTimeSlots = () => {
    startGettingTimeSlotTransition(async () => {
      ...
      setTimeSlots(
        resp.data.data.map((slot: any, idx: number) => {
          return {
            id: idx,
            date: slot[0],
            name: slot[1],
  ...
  const availableCourses = filteredCourses.filter(
    (course) => !assignedCourses.some((assigned) => assigned.id === course.id)
  );
  ...
  const handleDrop = (e: React.DragEvent, slotId: string) => {
    e.preventDefault();
    if (draggedCourse) {
      const isCourseOccupied = assignedCourses.some(
        (course) => course.id == draggedCourse.id
      );
      const isSlotOccupied = assignedCourses.some(
        (assigned) => assigned.slotId === slotId
      );
      //   if (isSlotOccupied) {
      ...
      const updatedAssignments = assignedCourses.filter(
        (assigned) => assigned.id !== draggedCourse.id
      );
  ...
  const removeCourseFromSlot = (courseId: number) => {
    setAssignedCourses(
      assignedCourses.filter((assigned) => assigned.id !== courseId)
    );
  };
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex-1">
                         <Grip className="translate-x-3.5"/>
                      <div>
        ...
        <div className="grid grid-cols-3 w-full gap-2 lg:col-span-2">
          {uniqueDates.map((date) => {
            ...
            return (
              <div
                key={date}
                className="border-2 border-gray-300 rounded-md overflow-hidden shadow-sm"
              >
                {/* Date Header */}
                ...
                <div>
                  {slotsForDate.map((slot) => {
                    ...
                    const slotCourses = assignedCourses.filter(
                      (course) => course.slotId == slot.id
                    );
                          ...
                          {assignedCourse ? (
                            <div className="rounded-lg p-1 shadow-sm border border-blue-200">
                              <div className="flex justify-between">
                                <div className=" p-1 font-bold border-2 rounded-sm border-gray-500">
                                  {slotCourses.length}
                                </div>
                                <button
                                  onClick={() =>
                                    removeCourseFromSlot(assignedCourse.id)
                                  }
