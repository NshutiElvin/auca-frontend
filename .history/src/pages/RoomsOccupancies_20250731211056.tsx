...
const OccupanciesPage = () => {
  ...
  try {
    if (dropAction === "group") {
      ...
      try {
        const verifyResponse = await axios.post("/api/rooms/verify_room_change/", {
          room: selectedRoom,
          courseGroup: draggedCourseGroup,
        });

        if (verifyResponse.status === 200) {
          const changeResponse = await axios.patch("/api/rooms/change_room/", {
            room: selectedRoom,
            courseGroup: draggedCourseGroup,
          });
          ...
          if (changeResponse.status === 201) {
            ...
            setDialogMessage(changeResponse.data?.message || "Room change failed");
            setDialogOpen(true);
          }
        } else {
          setDialogMessage(verifyResponse.data?.message || "Verification failed");
          setDialogOpen(true);
        }
      } catch (error:any) {
        setDialogMessage(error.response?.data?.message || "Verification error occurred");
        setDialogOpen(true);
      }
    }
...
if (dropAction === "students") {
  ...
  try {
    ...
    const verifyResponse = await axios.post("/api/rooms/verify_change_students/", {
      room: selectedRoom,
    ...
    if (verifyResponse.status === 200) {
      ...
      const changeResponse = await axios.patch("/api/rooms/change_students/", {
        room: selectedRoom,
        students: selectedStudents,
      });
      ...
      if (changeResponse.status === 201) {
        ...
        setDialogMessage(changeResponse.data?.message || "Student change failed after verification");
        setDialogOpen(true);
      }
    } else {
      setDialogMessage(verifyResponse.data?.message || "Student change verification failed");
      setDialogOpen(true);
    }
  } catch (error: any) {
    setDialogMessage(error.response?.data?.message || "Error during student room change");
    setDialogOpen(true);
  }
}
  } catch (error) {
  ...
  //     if (dropAction == "group") {
  //       if (!draggedCourseGroup) return;
  //       const response = await axios.post("/api/rooms/verify_room_change/", {
  //         room: selectedRoom,
  //         courseGroup: draggedCourseGroup,
  //       });

  //       if (response.status==200) {
  //         const resp = await axios.patch("/api/rooms/change_room/", {
  //           room: selectedRoom,
  //           courseGroup: draggedCourseGroup,
  //         });
  ...
  //     if (dropAction == "students") {
  //       if (selectedStudents.length <= 0) return;
  //       const response = await axios.patch("/api/rooms/change_students/", {
  //         room: selectedRoom,
  //         students: selectedStudents,
  //       });
