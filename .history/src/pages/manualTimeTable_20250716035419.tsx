...
import { Search, Clock, Calendar, X, SignalLowIcon } from "lucide-react";
...
function ManualTimeTable() {
  ...
  const [draggedCourse, setDraggedCourse] = useState<Course | null>(null);
    ...
    const [assignedCourse, setAssignedCourse] = useState<AssignedCourse| null>(null);
  ...
  const handleDrop = (e: React.DragEvent, slotId: string) => {
    ...
    if (isCourseOccupied) {
      ...
      setAssignedCourse(newAssignment)
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {availableCourses.map((course) => (
                <div
                  ...
                  className="p-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 hover:border-blue-300 cursor-move transition-all duration-200"
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-800 text-sm">{course.title}</h3>
                      <p className="text-xs text-gray-600 mt-1">{course.department.name}</p>
                      {course.code && (
        ...
        <div className="lg:col-span-2">
          <div className="space-y-6">
            {uniqueDates.map((date) => {
              ...
              return (
                <div key={date} className="bg-gray-100 rounded-lg p-6">
                  {/* Date Header */}
                  <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-800">{date}</h2>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    //   const Course = assignedCourse
                      ...
                      return (
                        ...
                        >
                          {/* Time Period Header */}
                          <div className="bg-gray-200 p-4 text-center border-b border-gray-300">
                            <h3 className="text-lg font-bold text-gray-800">{slot.name}</h3>
                          ...
                          <div
                            ...
                            className={`
                              ...
                              ${assignedCourse 
                                ? 'bg-blue-50' 
                                : 'bg-gray-50 border-2 border-dashed border-gray-400 hover:border-blue-400 hover:bg-blue-50'
                              }
                            `}
                          >
                            {assignedCourse ? (
                              <div className="bg-white rounded-lg p-4 shadow-sm border border-blue-200">
                                ...
                                <div className="flex justify-between items-start">
                                  <div className="flex-1">
                                    <h4 className="font-semibold text-gray-800">{assignedCourse.title}</h4>
                                    <p className="text-sm text-gray-600 mt-1">{assignedCourse.department.name}</p>
                                    {assignedCourse.code && (
                                      <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded mt-2 inline-block">
                                        {assignedCourse.code}
                                      </span>
            ...
            {assignedCourses.map((course) => {
              ...
              return (
                <div key={course.id} className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-sm text-blue-600 mt-1">{slot?.name} ({slot?.startTime} - {slot?.endTime} - {slot?.date})</p>
                </div>
              );
            })}
          </div>
