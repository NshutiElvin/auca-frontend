...
function ManualTimeTable() {
  ...
  console.log(suggestions, bestSuggestion);
  ...
  return (
    ...
    >
            ...
            return (
              ...
              >
                ...
                <div className="text-center mb-1">
                  <h2 className="text-lg font-bold">{format(dateObj, "eee")} </h2>
                  <p className="text-sm text-gray-500">
                    {format(dateObj, "dd MMM yyyy")} </p>
                </div>
                                    ...
                                    <Popover>
                                      ...
                                      >
                                        <ScrollArea className="h-[50vh] rounded-md border p-4">
                                          {moreExams.length > 0 ? (
                                            moreExams.map((exam, idx) => (
                                              <div
                                                key={idx}
                                                className="flex justify-between align-middle  space-x-1 flex-row-reverse"
                                              >
                                                <Badge
                                                  variant={"outline"}
                                                  className="w-full p-2   rounded flex-grow overflow-hidden font-medium flex-1 hover:opacity-45 hover:border-blue-300 cursor-move transition-all duration-200"
                                                  draggable
                                                  onDragStart={(e) =>
                                                  ...
                                                  onClick={(e) => {
                                                    if (slot.exams.length > 0) {
                                                      removeExamCourse(e, {
                                                        ...
                                                        courseId:
                                                          exam.group.course.id,
                                                      });
                                                    }
                                                  }}
                                                  variant={"outline"}
                                                  className="text-gray-500 hover:text-gray-400  p-1 rounded hover:bg-red-50 flex-0.2"
                                                >
                                                  <X className="w-4 h-4" />
                                                </Button>
                                              </div>
      ...
      {dialogOption?.option == "conflict" && (
        <DialogContent className="sm:max-w-[525px] md:max-w-[700px] max-h-[90vh] overflow-y-auto ">
          <DialogHeader className="text-center space-y-4 pb-2 flex items-center justify-center">
            <DialogTitle className="text-l font-bold  leading-tight">
              Conflicts Found
            </DialogTitle>
            ...
            <ScrollArea className="max-h-[50] h-fit rounded-md border p-4">
              {bestSuggestion ? (
                <Badge
                  variant={"outline"}
                  className="w-full p-2 m-1  rounded flex-grow overflow-hidden font-medium"
                >{bestSuggestion.slot} {format(new Date(bestSuggestion.date), "eee")}   {format(new Date(bestSuggestion.date), "dd MMM yyyy")}
          ...
          <DialogDescription className="text-sm text-gray-600 max-w-md mx-auto leading-relaxed  text-center">
            The other Suggested slots.
          </DialogDescription>
          <div className="flex flex-col gap-2">
            <div className="max-h-[50] h-fit rounded-md border p-4">
              {suggestions && suggestions.length > 0 ? (
                <Select>
                  ...
                  <SelectContent>
                    <SelectGroup>
                      <SelectLabel>Best Suggested Slot</SelectLabel>
                      <SelectItem
                    ...
                    <SelectGroup>
                   <ScrollArea  className="max-h-[50vh] mr-5">
                       <SelectLabel>Other Suggested Slots</SelectLabel>
                      ...
                      {suggestions.map((suggestion, idx) => (
                        ...
                        >
                          {suggestion.slot} {format(new Date(suggestion.date), "eee")}   {format(new Date(suggestion.date), "dd MMM yyyy")}
                        </SelectItem>
                      ))}
                   </ScrollArea>
                    </SelectGroup>
