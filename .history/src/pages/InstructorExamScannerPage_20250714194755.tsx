...
function InstructorExamScannerPage() {
  ...
  const performVerification = (data:QrCodeData) => {
    startVerifyingTransition(async () => {
      try {
        const response = await axios.post("/api/exams/student-exam/verify/", scannedData);
  ...
  const areQrCodeDataValid = (data: QrCodeData) => {
  ...
  const handleQrCodeDetected = (result: any) => {
    ...
    if (result) {
      try {
       ...
       if  (areQrCodeDataValid(parsedData)){
          setDialogOpen(true);
