...
import { Search, Clock, Calendar, X, Grip, Trash, CalendarIcon } from "lucide-react";
...
function ManualTimeTable() {
  ...
  const [existingDraggedExamGroup, setExistingDraggedExamGroup] = useState<MyExam[]>([]);
  ...
  const handleCourseGroupDragStart = (e: React.DragEvent, group: any) => {
    setDraggedCourseGroup({...group, group:group.real_group});
    console.log(group)
    e.dataTransfer.effectAllowed = "move";
  };

   const handleExistingCourseGroupDragStart = (e: React.DragEvent, group: any) => {
    setExistingDraggedExamGroup({...group, group:group.real_group});
    e.dataTransfer.effectAllowed = "move";
  };
                          ...
                          {slot.exams ? (
                            <div className="flex-grow flex flex-col gap-2 w-full">
                              ...
                              <div className="flex justify-between items-start">
                                ...
                                <PopoverContent className="max-w-[500px] p-0" align="center">
                                   <ScrollArea className="h-[50vh] rounded-md border p-4">
                                          {moreExams.length > 0 ? (
                                            moreExams.map((exam) => (
                                              ...
                                              >
                                                {exam.course} - Group {exam.group.group_name}
                                              </Badge>
                                            ))
                                          ) : (
