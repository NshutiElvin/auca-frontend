...
const DashboardPage = () => {
  ...
  const renderCardDataItem = (item: any, index: number) => {
    console.log(item)
  return (
    <tr key={index} className="border-b hover:cursor-pointer hover:bg-muted-foreground/10 transition-colors">
      <td className="p-4">
      ...
      <td className="p-4 text-sm text-muted-foreground">
        {item.student_count !== undefined ? item.student_count : '-'}
      </td>
      
      <td className="p-4">
        {item.status ? <StatusButton status={item.status} /> : '-'}
      </td>
