          ...
          for (const schedule of room.schedules) {
            for (const exam of schedule.exams) {
              ...
              flatData.push({
                ...
                course_department:exam.exam_department,
                course_semester:exam.exam_semester
              });
            }
          }
        }
            ...
            return (
              ...
              >
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-black text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
                  {course.course_code} {course.course_semester}: {course.student_count} students ({capacityPercentage.toFixed(1)}%)
                </div>
              </div>
            );
          })}
        ...
        {slot.courses.map((course: any, courseIndex: number) => (
          <div key={courseIndex} className="flex items-center space-x-2">
            ...
            <span className="text-xs">
              {course.course_code} {course.course_department}: {course.student_count} students
            </span>
          </div>
        ))}
      </div>
