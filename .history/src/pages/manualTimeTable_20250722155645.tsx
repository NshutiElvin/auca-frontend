...
function ManualTimeTable() {
  ...
  const processDroppedCourse= async(e: React.DragEvent, slotInfo:any)=>{
    e.preventDefault();
    try {
      setServerLoadingMessage({message:`New exam added to ${slotInfo.day} ${slotInfo.slot.name}`,isServerLoading:true})
                  ...
                  {date.exams.map((slot, idx) => {
                    return (
                      <Card
                        ...
                        onDrop={(e) =>
                          handleDrop(e, { day: date.day, slot: slot })
                        }
                      >
                        {/* Time Period Header */}
