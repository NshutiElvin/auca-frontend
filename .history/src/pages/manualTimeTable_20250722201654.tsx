...
import { Search, Clock, Calendar, X, Grip, Trash } from "lucide-react";
...
function ManualTimeTable() {
  ...
  const{setServerLoadingMessage, setToastMessage, serverLoadingMessage}= useToast()
  ...
  const [moreExams, setMoreExams] = useState<Exam[]>([]);
  ...
  const handleDroppedCourse= async(e: React.DragEvent, slotInfo:any)=>{
    ...
    try {
      setServerLoadingMessage({message:`Processing`,isServerLoading:true})
      const resp= await axios.post("/api/exams/exams/add-exam-to-slot/", {...slotInfo, course_group: draggedCourse})
  ...
  const scheduleCourse=async()=>{
    try {
      setDialogOpen(false)
      setServerLoadingMessage({message:`Scheduling Exam`,isServerLoading:true})
      const resp= await axios.post("/api/exams/exams/schedule-course-group/", {...selectedSlotInfo, course_group: draggedCourse})
  ...
  const removeCourseFromSlot = (slotInfo: any) => {
    setServerLoadingMessage({message:"Exam removed",isServerLoading:true})
  ...
  return (
    <Dialog open={dialogOpen} onOpenChange={setDialogOpen} >
      <div className={`grid grid-cols-1 ${unscheduledExams.length>0&&"lg:grid-cols-3"} gap-2 ${serverLoadingMessage?.isServerLoading && "pointer-events-none opacity-20"}`}>
        {unscheduledExams.length>0&&<div className="lg:col-span-1">
                                  ...
                                  onClick={() => {
                                    if (slot.exams.length > 0) {
                                      ...
                                      removeCourseFromSlot({
                                        ...
                                        courseId: lastExam.courseId, // Make sure this is passed
                                      });
                                    }
                                  }}
                                  variant={"outline"}
                              ...
                              <div className="flex justify-between items-start">
                                <div className="flex-1">
                                  ...
                                  {slot.exams[slot.exams.length - 1] && (
                                    <Badge
                                      variant={"outline"}
                                      className="w-full p-2 m-0.3 rounded flex-grow overflow-hidden"
                                      onClick={() => {
                                        setMoreExams(
                                          slot.exams.map((e: any) => {
                                            return {
                                    ...
                                    >
                                      {slot.exams.length > 0 &&
                                        slot.exams[slot.exams.length - 1].group
                                          .course.title}
                                    </Badge>
                                  )}
                                </div>
     ...
     {dialogOption?.option=="more" &&  <DialogContent className="sm:max-w-[425px] md:max-w-[500px] max-h-[90vh] overflow-y-auto ">
      ...
      {dialogOption?.option=="conflict"&&  <DialogContent className="sm:max-w-[425px] md:max-w-[500px] max-h-[90vh] overflow-y-auto ">
        <DialogHeader className="text-center space-y-4 pb-2 flex items-center justify-center">
          <DialogTitle className="text-l font-bold  leading-tight">
            Conflicts Found
          </DialogTitle>
          ...
          <DialogDescription className="text-sm text-gray-600 max-w-md mx-auto leading-relaxed  text-center">
            The conflict that restrain this course from scheduled on this date.
          </DialogDescription>
        </DialogHeader>
              ...
              <ScrollArea className="max-h-[50] h-fit rounded-md border p-4">
                {conflictMessage && conflictMessage.length > 0 ? (
                  conflictMessage.map((conflictedGroup) => {
                    ...
                    >
                      {conflictedGroup[0].course.title} {conflictedGroup[0].group_name} - {conflictedGroup[1].course.title} {conflictedGroup[1].group_name} - {conflictedGroup[2]} 
                    </Badge>
})
                ) : (
                  <div className="flex flex-col items-center justify-center py-6 text-center">
                    <Trash className="h-12 w-12 text-primary mb-2" />
                    <p className="text-lg font-medium text-primary">No  conflicts</p>
        ...
        <DialogFooter>
          ...
          <Button variant={"outline"} onClick={scheduleCourse}>Yes</Button> <Button variant={"secondary"} onClick={()=>{
            setDraggedCourse(null)
            setDialogOpen(false)
            setSelectedSlotInfo(null)
          }}>No</Button>
