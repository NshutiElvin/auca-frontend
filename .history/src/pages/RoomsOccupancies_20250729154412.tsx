...
import { ChevronLeft, ChevronRight, Calendar, Table, MoreHorizontal, Bell, User, X } from 'lucide-react';
...
const BlockedBookings = () => {
  ...
  const fetchOccupancies = () => {
    startTransition(async () => {
      try {
        ...
        setData(mockData);
        setExamsDates(new Set(mockData.map(item => item.date)));
      } catch (error) {
  ...
  const OccupancyCard = ({ occupancies }: { occupancies: RoomOccupancy[] }) => {
    const isOvercapacity = occupancies.reduce((sum, occ) => sum + occ.student_count, 0) > (occupancies[0]?.room_capacity || 0);
        ...
        <div className="flex items-center justify-between h-full">
          <div className="flex-1 min-w-0">
            ...
            <div className={`text-xs truncate ${
              ...
              {occupancies.reduce((sum, occ) => sum + occ.student_count, 0)} students
            </div>
          </div>
                  ...
                  {timeSlots.map((_, timeIndex) => {
                    ...
                    return (
                      <div key={timeIndex} className="flex-1 p-2 border-r border-gray-200 last:border-r-0 min-h-[64px]">
                        {occupancies.length > 0 && <OccupancyCard occupancies={occupancies} />}
                      </div>
                    );
                  })}
                </div>
